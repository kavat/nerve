...
class SchemaParser:
  def __init__(self, data, req):
    ...
    rds.store_scan_info(data['type'], base64.b64encode(json.dumps(self.metadata).encode('ascii')).decode('utf-8')) 
...
class ConfParser:
  ...
  def get_package_type_config(self):
  ...
  def get_password_ssh_config(self):
    return self.values['password_ssh']
